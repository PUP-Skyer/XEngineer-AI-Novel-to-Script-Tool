...
export class OpenAiProvider extends BaseAiProvider {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.client = new OpenAI({
      apiKey: this.configService.get('ai.providers.openai.apiKey'),
      baseURL: this.configService.get('ai.providers.openai.baseUrl'),
    });
  }

  async complete(request: AiCompleteRequest): Promise<AiCompleteResponse> {
    const model =
      request.model || this.configService.get('ai.providers.openai.model');
